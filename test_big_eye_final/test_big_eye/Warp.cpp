...
Warp::Warp(int r, int S) :m_radius(r), m_strength(S)
{
}

void Warp::iiw( Point begin, double point_x, double point_y,
	double *src_x, double *src_y)
{
	...
	{
		double tb = xc / rs;
		double tc = m_strength / 50.0 * (3 * tb*tb*tb*tb - 4 * tb*tb*tb + 1);
		*src_x = point_x - tc * (point_x - begin.x);
		*src_y = point_y - tc * (point_y - begin.y);

	}
}
void Warp::warping(Mat&src, Point center)
{
	...
	for (int i = center.y - m_radius; i < center.y + m_radius; i++)
		for (int j = center.x - m_radius; j < center.x + m_radius; j++)
		{
			if (i >= 0 && i < h && j >= 0 && j < w)
			{
				double src_x;
				double src_y;
				iiw(Point(center.x, center.y), j, i, &src_x, &src_y);
